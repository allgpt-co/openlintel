...
export const growthPages = [
  ...
  page(
    ...
    'How this OpenLintel website handles pilot requests, optional analytics, attribution, and your privacy choices.',
  ),
  page(
    'pilot-thanks',
    'pilot/thanks/',
...
const contact = (settings) =>
  settings.contactEmail
    ...
    : `<a href="${config.repo}/issues">the public repository issue tracker</a> (do not post personal or client information)`;
  ...
  switch (record.id) {
    ...
    case 'product-status':
      return (
        ...
        `<div class="wrap growth-prose">${section('Available on this website', '<ul><li>Editable DOCX and XLSX resources, with blank templates and illustrative examples where listed.</li><li>Guides explaining residential design documentation and coordination.</li><li>The Window Room, a fictional example with drawings and a partial material schedule.</li><li>Public source code and development documentation.</li></ul><p>Check each download label for its actual format. The presentation resource is a DOCX storyboard, not a PowerPoint or Canva file.</p>')}${section('Illustrative, not verified application output', '<p>The sample room, AI-generated concept imagery, quantities, prices, drawings, and records are teaching examples. They do not establish that the application generates a complete coordinated package or has been used on a customer project.</p><p>Unresolved fields and pending-review labels are part of the example. Do not use it for construction, purchasing, permitting, or a site-specific technical decision.</p>')}${section('In active development', `<p>OpenLintel explores connections between room inputs, design exploration, drawings, and material information. Source modules are not a promise that every workflow, export, integration, or provider is production-ready.</p><p>Before evaluating a capability, check the current <a href="${config.repo}">repository</a> and development documentation, pin a revision, and reproduce the specific workflow. A working result should be labeled with its version and limitations.</p>`)}${section('Not offered or established here', '<ul><li>A functioning hosted trial, guaranteed pilot admission, or support service.</li><li>Permit-ready or construction-ready output, compliance certification, or exact quantities.</li><li>Verified customer outcomes, adoption figures, testimonials, or time-saving claims.</li><li>Private or offline AI merely because the code can be self-hosted; configured providers may receive data.</li></ul>')}${section('Next step', `<p>Explore the ${link('sample project', 'sample-project/')}, inspect the ${link('open-source project', 'open-source/')}, or read about a ${link('pilot discovery conversation', 'pilot/')}.</p>`)}</div>`
      );
    case 'about':
      return (
        intro(
