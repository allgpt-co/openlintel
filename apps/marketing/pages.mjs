...
export function home(project) {
  return `<section class="hero wrap"><div class="hero-copy">${eyebrow('For the people who shape spaces')}<h1>Your vision.<br>In every <em>detail.</em></h1><p class="hero-description">From the first possibility to the decisions that make it real. AI-assisted design, drawings, and material planning for residential design professionals.</p><div class="hero-actions">${button('Explore a sample project', 'sample-project/')}${textLink('See how it works', 'how-it-works/')}</div><p class="hero-footnote"><span class="status-dot"></span> Open source. Made for considered design.</p></div><figure class="hero-visual">${picture('room', roomAlt, { hero: true, sizes: '(max-width: 800px) 100vw, 60vw' })}<div class="image-label"><span class="label-line"></span>The Window Room<span class="micro">A study in light, space & everyday life</span></div><span class="image-index">ILLUSTRATIVE PROJECT / 001</span></figure><div class="hero-baseline"><span>Design is in the details. Keep them connected.</span><a href="#the-story">Discover the story <span aria-hidden="true">↓</span></a></div></section>
  ...
  <section class="connection-section wrap"><div>${eyebrow('From intention to information', '03 /')}<h2>Give each decision<br>a place to <em>go.</em></h2><p class="body-copy">The concept, the drawing, the material reference. OpenLintel brings them into a shared project workflow, with your judgment at each step.</p>${textLink('Explore the workflow', 'how-it-works/')}<div class="reference-note"><span class="eyebrow">A detail, connected</span><p>Oak storage <span>→</span> J-01 <span>→</span> Drawing WR-02</p></div></div><figure class="plan-figure"><img src="${url('assets/downloads/window-room-plan.svg')}" width="900" height="700" loading="lazy" alt="The Window Room furnished plan with linked material and furniture references">${caption('FIG. 02 — QUIET OAK / FURNISHED PLAN', 'Illustrative · Pending review')}</figure></section>
  ...
  <section class="open-band"><div class="wrap"><div class="open-symbol" aria-hidden="true">[ &nbsp; ]</div><div>${eyebrow('Open by design')}<h2>An open foundation<br>for your practice.</h2></div><div><p>Explore the source. Understand the project. Help shape what comes next.</p>${textLink('Meet the open-source project', 'open-source/')}</div></div></section>${cta()}`;
}
...
export function workflow(project) {
  const steps = [
    ...
    [
      ...
      'Material quantities, categories, and estimates in the app workflow',
  ...
  const faqs = [
    ...
    [
      ...
      'The designer reviews the brief, selects a direction, and checks the drawings and materials. Generated drawings are prepared for professional review.',
    ],
    [
