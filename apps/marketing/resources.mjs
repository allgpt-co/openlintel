...
function card(page) {
  return `<article class="resource-card"><p class="resource-kind">${page.kind === 'template' ? `Editable ${page.format.toUpperCase()}` : 'Practical guide'}</p><h3><a href="${url(page.path)}">${esc(page.title)}</a></h3><p>${esc(page.description)}</p><span class="resource-card-link" aria-hidden="true">${page.kind === 'template' ? 'Preview & download' : 'Read the guide'} →</span></article>`;
}
...
function visual(page, project) {
  if (page.visual === 'mood-boards')
    return `<div class="board-pair">${project.concepts.map((concept) => `<figure class="annotated-board">${picture(concept.image, `${concept.name}: AI-generated concept for the same illustrative room`, { hero: true, sizes: '(max-width: 800px) 100vw, 40vw' })}${caption(concept.name, concept.selected ? 'Selected illustrative direction' : 'Alternative study only')}<p>${esc(concept.description)}</p><div class="board-labels">${(concept.selected ? ['FIN-01 · Retained oak floor', 'J-01 · Pale oak joinery', 'Linen · Proposed upholstery'] : ['Existing floor retained', 'Darker joinery study', 'Olive upholstery study']).map((s) => `<span>${esc(s)}</span>`).join('')}</div></figure>`).join('')}</div>`;
