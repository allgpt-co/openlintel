...
const configPath = resolve(output, 'video-smoke.config.json');
...
const browserCodePath = resolve(output, 'video-browser-smoke.code');
writeFileSync(
  browserCodePath,
  readFileSync(resolve(source, 'video-browser-smoke.js'), 'utf8').trim().replace(/;$/, ''),
);
const cli = (...args) => {
  const result = execFileSync(
    ...
    [
      ...
      'playwright-cli',
      '-s=openlintel-video-smoke',
      ...args,
    ],
    { cwd: root, encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 },
  );
  process.stdout.write(result);
  if (/### Error/.test(result)) throw new Error('Video browser verification reported an error');
};
try {
  cli(
    'open',
