...
async function generateRenderImage(
  ...
  try {
    ...
    if (opts.renderType === 'panorama_360') {
      ...
      const imageUrl = await callDalle(prompt, '1792x1024', dalleQuality);
      ...
      for (const a of angles) {
        ...
        const url = await callDalle(prompt, '1792x1024', dalleQuality);
        frameUrls.push(url);
      ...
      const beforeUrl = await callDalle(beforePrompt, '1792x1024', dalleQuality);
      const afterUrl = await callDalle(afterPrompt, '1792x1024', dalleQuality);
...
async function callDalle(_prompt: string, _size: string, _quality: string): Promise<string> {
  throw new Error(
    'Image generation is not supported by Bedrock Converse/Kimi K2.5. Use a Bedrock image model integration for render workflows.',
  );
}
